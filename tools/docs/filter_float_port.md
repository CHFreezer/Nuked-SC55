...
**日期**：2026-09-05
**状态**（2026-09-06）：S1 **已实现 + 两轮 A/B，已整合（`-floatsat` 已并入 `-float` 并删除）**。
- **`-float`**（现行为＝逐加法饱和）：filter 内部 float，**每个 stage 计算后钳 ±1.0**（mk1 语义、float 精度，状态不量化到 20-bit），输出硬钳 ±1.0 + ×524288 截断回 20-bit。**对全部 chip 生效——`-float` 是第一分支，覆盖 `-mk1`/`-mk2`**。
- 历史 A/B（已归档）：早期 `-float` 是「只在输出钳、内部不钳」，**用户听感：还是爆音** → 排除「32-bit 太窄溢出」；随后加「逐加法饱和」（即 `-floatsat`，**不爆音**，与 `-mk1` 同为干净）确认为根因，遂把饱和并入 `-float`、删除 `-floatsat`。
- 改动：`pcm.cpp`（`clamp20` + `sats` helper、`if (pcm_float)` 分支全 stage 内联 `sats()`、`!active` 清 `fstate`）、`pcm.h`（`fstate[32][2]`）、`mcu.cpp`（`pcm_float` + `-float` CLI + help）、`mcu.h`（extern）。ninja 增量编译通过（clang-cl Release），`-float` 跑 5s 无崩溃（filter 每 slot 无条件执行，实跑到）。

**诊断逻辑（已坐实，2026-09-06）**：A/B 四象限——**两爆音版（`-mk2`、`-mk2 -float`(旧·内部无饱和)）共同点＝filter 内部无饱和；两干净版（`-mk1`、`-floatsat`）＝逐加法饱和**。跟字长（20/32/float）无关，只跟有没有饱和有关。**结论：mk2 hack 的「无饱和」（`// hack: use 32-bit math to avoid overflow`）就是爆音根因**；真 mk2 芯片应像 mk1 一样逐加法饱和（把 ring-up 驯成有界极限环）。**剩余次要变量＝float 精度**（`-float` vs `-mk1`：反馈不量化到 20-bit）是否可闻 → 待用户听感；若可闻则是「mk1 稳定 + float 精度」甜点，若不可闻则直接给 mk2 补回饱和最省事。**对比命令**：`build\nuked-sc55.exe -mk1` / `-mk2` / `-float`（`-float` 覆盖两者）。

---

## 0. 结论速览

- **数值格式**：filter 内部 float（宽动态范围 / 类 "HDR"），**输出硬钳 ±1.0**（跟 Roland VSTi 同机制——**硬 clamp，非 soft tone curve**），再 **×524288 截断**回 20-bit。
