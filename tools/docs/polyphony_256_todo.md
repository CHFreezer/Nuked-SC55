...
> **历史范围（2026-09-13）**：本表的补丁任务与勾选结果属于旧扩容实验，不是当前待办。
> 当前保留 M4 检查点，先完成 [M4.5](../../mk2cpp/docs/13_m45_native_reconstruction.md)，再做 M5 真 256。
> 下文 ROM/页布局证据可复用；旧 CLI、补丁实现与基线目录要求不规定原生终态。
...
### 255 路线（妥协上限，目标 256；D1，见 `polyphony_256_feasibility.md`）
...
**R12 上限 ✅ 2026-09-11：N=255（妥协上限；目标 256）（mk2 §6.5 R12）**
...
- [x] 结论：**N=255**（0xff 哨兵 + a42d 回绕）；A 符号测试不改则 N≤128（B 重写修）。**255 是 0xff 哨兵 + 8-bit 计数的妥协上限，非最终目标（目标 256；真 256 需改哨兵表示/加宽计数）**
...
**R14 设计取舍 ✅ 结论：继续 (ii)；本期妥协上限 255（目标 256，非最终目标）；`-voices:<n>` 28–255 真生效（决策 2026-09-11；口径修订 2026-09-11）**
...
- [x] **决策**：flag=`-voices:<n>`（单横线冒号，28–255）；B 由 C 生成器按 n 参数化（界/config 印入）；GT `reg_slots` 用模式开关（默认公式不动，扩展 `val+1`）；验证 28/255 必测 + 中间值抽测（255 为妥协上限，目标 256）
