// Standalone SM sub-CPU (ported verbatim from nukeykt submcu.cpp, no external deps).
...
// Shared UART buffer (defined in h8vm_sm.c so the SM and main share one buffer).
uint8_t sm_uart_buffer[SM_UART_BUFFER_SIZE];
uint32_t sm_uart_read_ptr;
...
// Get-and-clear the pending GA line (read by the main MCU via 0xe402).
int SM_GATrigger(void)
{
    int t = ga_int_trigger;
    ga_int_trigger = 0;
    return t;
}
...
uint8_t SM_SysRead(uint32_t address)
{
    ...
    else if (address >= 0xf8 && address < 0xfc)
    {
        ...
        sm_device_mode[SM_DEV_IPCE0 + (address & 3)] = 0;
        return val;
    }
    else if (address == 0xff)
    {
...
void SM_Opcode_SEI(uint8_t opcode)
{
    SM_SetStatus(1, SM_STATUS_I);
}

void SM_Opcode_CLD(uint8_t opcode)
{
    SM_SetStatus(0, SM_STATUS_D);
}

void SM_Opcode_CLT(uint8_t opcode)
{
    SM_SetStatus(0, SM_STATUS_T);
}

void SM_Opcode_LDX(uint8_t opcode)
{
    uint8_t val = 0;
    switch (opcode)
    {
...
void SM_Opcode_LDY(uint8_t opcode)
{
    uint8_t val = 0;
    switch (opcode)
    {
...
void SM_Opcode_TXS(uint8_t opcode)
{
    sm.s = sm.x;
}

void SM_Opcode_TXA(uint8_t opcode)
{
    sm.a = sm.x;
    SM_Update_NZ(sm.a);
}

void SM_Opcode_STA(uint8_t opcode)
{
    uint16_t dest = 0;
    switch (opcode)
    {
...
void SM_Opcode_INX(uint8_t opcode)
{
    sm.x++;
    SM_Update_NZ(sm.x);
}

void SM_Opcode_INY(uint8_t opcode)
{
    sm.y++;
    SM_Update_NZ(sm.y);
}
...
void SM_Opcode_CPX(uint8_t opcode)
{
    uint8_t operand = 0;
    switch (opcode)
    {
...
void SM_Opcode_CPY(uint8_t opcode)
{
    uint8_t operand = 0;
    switch (opcode)
    {
...
void SM_Opcode_BEQ(uint8_t opcode)
{
    int8_t diff = SM_ReadAdvance();
...
void SM_Opcode_BCC(uint8_t opcode)
{
    int8_t diff = SM_ReadAdvance();
...
void SM_Opcode_BCS(uint8_t opcode)
{
    int8_t diff = SM_ReadAdvance();
    if ((sm.sr & SM_STATUS_C) != 0)
        sm.pc += diff;
...
void SM_Opcode_LDM(uint8_t opcode)
{
    uint8_t val = SM_ReadAdvance();
    SM_Write(SM_ReadAdvance(), val);
}

void SM_Opcode_LDA(uint8_t opcode)
{
    uint8_t val = 0;
    switch (opcode)
    {
...
void SM_Opcode_CLI(uint8_t opcode)
{
    SM_SetStatus(0, SM_STATUS_I);
}

void SM_Opcode_STP(uint8_t opcode)
{
    sm.sleep = 1;
}

void SM_Opcode_PHA(uint8_t opcode)
{
    SM_PushStack(sm.a);
}
...
void SM_Opcode_RTI(uint8_t opcode)
{
    sm.sr = SM_PopStack();
    sm.pc = SM_PopStack();
    sm.pc |= SM_PopStack() << 8;
...
void SM_Opcode_PLA(uint8_t opcode)
{
    sm.a = SM_PopStack();
    SM_Update_NZ(sm.a);
}

void SM_Opcode_BRA(uint8_t opcode)
{
    int8_t disp = SM_ReadAdvance();
    sm.pc += disp;
}

void SM_Opcode_JSR(uint8_t opcode)
{
    uint16_t newpc = 0;
    switch (opcode)
    {
...
void SM_Opcode_CMP(uint8_t opcode)
{
    uint8_t operand = 0;
    switch (opcode)
    {
...
void SM_Opcode_BNE(uint8_t opcode)
{
    int8_t diff = SM_ReadAdvance();
    if ((sm.sr & SM_STATUS_Z) == 0)
        sm.pc += diff;
...
void SM_Opcode_RTS(uint8_t opcode)
{
    sm.pc = SM_PopStack();
    sm.pc |= SM_PopStack() << 8;
}

void SM_Opcode_JMP(uint8_t opcode)
{
    switch (opcode)
    {
        case 0x4c:
...
void SM_Opcode_ORA(uint8_t opcode)
{
    uint8_t val = 0;
    uint8_t val2 = 0;
...
void SM_Opcode_DEC(uint8_t opcode)
{
    uint8_t val = 0;
    uint16_t dest = 0;
    switch (opcode)
...
void SM_Opcode_TAX(uint8_t opcode)
{
    sm.x = sm.a;
    SM_Update_NZ(sm.x);
}

void SM_Opcode_STX(uint8_t opcode)
{
    uint16_t dest = 0;
    switch (opcode)
    {
...
void SM_Opcode_STY(uint8_t opcode)
{
    uint16_t dest = 0;
    switch (opcode)
    {
...
void SM_Opcode_SEC(uint8_t opcode)
{
    SM_SetStatus(1, SM_STATUS_C);
}

void SM_Opcode_NOP(uint8_t opcode)
{
}

void SM_Opcode_BPL(uint8_t opcode)
{
    int8_t diff = SM_ReadAdvance();
...
void SM_Opcode_CLC(uint8_t opcode)
{
    SM_SetStatus(0, SM_STATUS_C);
}

void SM_Opcode_AND(uint8_t opcode)
{
    uint8_t val = 0;
    uint8_t val2 = 0;
...
void SM_Opcode_INC(uint8_t opcode)
{
    uint8_t val = 0;
    uint16_t dest = 0;
    switch (opcode)
...
void SM_UpdateUART(void)
{
    if ((sm_device_mode[SM_DEV_UART1_CTRL] & 4) == 0) // RX disabled
        return;
    if (sm_uart_write_ptr == sm_uart_read_ptr) // no byte
        return;
    ...
    uart_rx_byte = sm_uart_buffer[sm_uart_read_ptr];
    sm_uart_read_ptr = (sm_uart_read_ptr + 1) % SM_UART_BUFFER_SIZE;
    uart_rx_gotbyte = 1;
    sm_device_mode[SM_DEV_INT_REQUEST] |= 0x40;
