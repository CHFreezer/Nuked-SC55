...
**原 wave ROM 模式对照（待实现）**：原生端加载提取后的参数表和用户指定的 wave ROM，
同配置 stock 28 与 GT 比较控制事件、PCM 样本及帧数。采集位置为增益/限幅后、SDL 转换前，
保持相同时间原点，不移位或放宽容差。原整数/`-float` 模式各自与同配置参考比较，见
[14 §8.1](../docs/14_m45_native_architecture.md#81-原-wave-rom-模式的-bit-对照)。

**透明 bank 检查（待实现）**：按 [16](../docs/16_transparent_bank.md) 确认元数据/波形
提取正确，JSON + WAV 能加载、试听与替换，修改实际生效。未编辑原厂素材保持无可闻
差异，不要求 bit 一致；沿用已有回归和用户试听，不新增可逆证明或复杂听觉流程。
透明 bank 模式启动至保存恢复均无需原 ROM；两种音源选择都不执行 H8/SM 或 gen/hand。

以下旧脚本的命令、整 WAV/hash 与 PC 门禁保持原行为，尚未覆盖上述原生目标。
