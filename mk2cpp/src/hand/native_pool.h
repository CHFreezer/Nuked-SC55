 ...
 * page-0 SRAM and is accessed with MCU_Read/MCU_Write, so the untranslated ROM
 * (and the state hashdump) observes exactly the stock bytes. The old native
 * Pool mirror, the per-PC L0 registration and the self-check are gone; the
 * alloc/free routines have their own hand module (native_allocfree.cpp) and
 * are aggregated in MK2CPP_PoolFillTables below.
 *
