...
口径：验收统一 `-voices:255`（"256 复音"指容量，活性上限 255）；音频/听感窗口 ≥300M，状态 ≥200M；
...
7. 口径已统一：CLI/`PCM_MAX_VOICE` 上限是 **255**（`pcm.h:27`、`mcu.cpp:1996`、
   help `mcu.cpp:2162`），R12 结论也是 N=255；验收统一 `-voices:255`（"256"指容量），
   两者不冲突（活性上限 vs 槽位容量，out/m4/12 §5.2），是否放开到 256 见 §6 D1。
...
9. 铁律的自动/人工分界：**自动** = trace/hash/pcmtrace/wav/audiohash 对比 + 判据解析；
   **必须人工在场** = LCD 画面/声音观察、n=28 听 A/B、255 压力听丢音/爆音。
...
10. **与 07/08/09 的对齐**：① 抓取命名采用 08 的 `-wav:<file>`，本文加窗口/哈希/元数据
    ...
    **`-mk2cpp-hand:0` vs hand-on 同二进制 A/B**（§2.1 可选第三跑）；
    ④ 验收口径统一 `-voices:255`（"256"指容量；07 §0.5 亦给活性上限 255），是否扩到
    256 见 §6 D1；⑤ L1 启用时机见 §6 D7，影响 trace gate 是否可用。
...
## 3. n=255 压力测试设计（容量 256，CLI 上限 255）
...
| # | 事项 | 状态/说明 |
|---|---|---|
| **D1** | 容量 256 是否/何时放开 | 口径不冲突（out/m4/12 §5.2）：`-voices:255` 是活性上限（`PCM_MAX_VOICE`），"256"是槽位容量（数组 260，slot 255 哨兵）；方案 A 下 `cfg3d` 恒 `0x7b`、与 256 无关。若把 `-voices:256` 当硬指标：需放开 clamp、数组 ≥264/EFF 260..263、确认 0xff 不是池哨兵（R12）与 mask bit255 的表示，属设计变更；本期不做 |
