...
> **适用范围更新（2026-09-13）**：本文保留 M4 指令集成与早期 M5 研究。M4 现以
> 逐指令整理/回归检查点收口；未完成的原生算法目标归入
> [M4.5](13_m45_native_reconstruction.md)，完整重建后再进入 M5。
> 下文 L0、PC、寄存器镜像和每指令调度约束适用于参考/迁移层，不规定原生业务层的
> 终态架构；旧 256 硬件窗口方案和 255 阶段上限也不是新路线的强制实现或最终验收。
> 有冲突的阶段状态与终态要求以 [00 计划](00_plan.md) 和 13 为准，ROM 证据继续复用。
