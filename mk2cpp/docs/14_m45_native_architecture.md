...
格式采用版本化的头部和明确的数据区，记录原素材哈希、导入器版本、型号/格式版本、
目录与 payload 校验值；引用越界、缺少必需区或配置不匹配应明确失败，不尝试 ROM 回退。
...
原生算法使用 bank 内的类型化采样定义，设备适配层将 bank 内的原编码只读波形数据
提供给现有 PCM；这里的芯片波形地址有实际用途，与 H8 的程序地址无关。
...
| 对象 | 责任 |
|---|---|
| `WaveData` / bank 只读资产存储 | 来源标识、解扰后的编码字节、PCM 地址映射；同一份资产由所有 voice 共享 |
...
M4.5 必须生成并使用**原编码 payload + 类型化目录/数据表 + 版本/来源哈希**的 bank。
零原 ROM 运行不要求破坏波形编码或重新录制采样；也不要求每个采样一个文件。
...
S4 至少完成目标场景所需的导入器、bank 与装载路径，对照原参数选择和 PCM 地址/控制
写入；S5/S6 补齐全部所需数据并完成无原 ROM 的运行验收。全库 WAV 导出不列为门禁。
...
这两项是独立且必须同时通过的门禁。更换数据容器不改变声音等价目标，也不证明
原生算法/时序已经恢复。按以下顺序验证：
