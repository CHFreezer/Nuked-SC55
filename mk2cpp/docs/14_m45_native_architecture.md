...
**M4.5 要恢复采样与音色数据的含义，不要求先把 wave ROM 全部转换为线性 PCM bank。**
读取二进制波形资产本身不构成 MCU 模拟。原生算法可以使用类型化的采样定义，设备
适配层继续将原编码的只读波形数据提供给现有 PCM；这里的芯片波形地址有实际用途。
...
| 对象 | 责任 |
|---|---|
| `WaveRomImage` / 只读资产存储 | 原文件身份、解扰后的编码字节、地址映射；同一份资产由所有 voice 共享 |
| `SampleDefinition` / 采样目录 | 稳定 SampleId、已确认的数据引用、起止/循环位置、播放模式及静态解码初始条件 |
...
业务层按 ID 与已证实参数工作；ROM 数据适配负责引用转换，PCM port 负责设备编码。

如以后需要自己的 bank 文件，可先封装**原编码 payload + 类型化目录 + 版本/来源哈希**，
作为可重建资产缓存；M4.5 允许直接从原 ROM 建立同样的内存视图，无需先实现打包器。
只把两个 bin 换后缀不算数据语义恢复。容器格式与是否预解码是两个独立决策。

研究工具可选导出线性 PCM/WAV、波形图和循环预览，标明解码初值、播放方向/速率、
数值精度与已验证范围。16-bit WAV 预览不能充当 20-bit 内部行为的无损依据。
导出结果不自动进入生产播放链；若以后使用预解码缓存，要单独证明解码/插值输出、
循环边界、变速与状态恢复等价，且不能凭该缓存移除原 PCM 的其余合成行为。

S4 至少为目标场景建立可读采样/音色目录，对照原参数选择和 PCM 地址/控制写入；
S5/S6 再按运行范围补齐相关表和路径。全库 WAV 导出不列为 M4.5 前置门禁。
