...
| PCM 保持原实现 | 原生核心产生参数与命令，适配层驱动 `PCM_Read/Write/Update`，保留读取副作用 |
...
```mermaid
flowchart TD
    Host[现有程序：CLI / ROM / RtMidi / SDL / LCD] --> Select{启动时选择}
...
| `GtPcmPort` 背后的现有 `pcm` | 芯片 RAM、latch、mask、IRQ、滤波/效果及采样状态 | 原生音符分配策略 |
...
在现有主程序完成参数/ROM 选择后确定后端，在 MCU 固件 reset/执行之前走不同的
控制初始化路径。共享资源、PCM、SDL、LCD 的初始化继续复用，保持默认分支原顺序。
...
目标 ROM 范围由已验证 MK2 资产配置决定，不能对其他型号静默运行这套算法。
...
`RomTables` 只解码波形/音色/参数/初始化表：字节序、记录长度、边界与资产版本在此
集中验证。业务层拿 `ToneDefinition` 等已确认字段的视图，避免遍布 `rom[address]`。
...
未提交设备操作、PCM 完整状态（含效果/浮点反馈）、LCD 状态及采集所需时间。
在 worker 的一致边界冻结，保存的 ID/操作类型不能包含 C++ 指针或回调地址。

默认分支保留旧快照格式；原生使用带后端、版本、ROM/PCM 配置标识的新容器。
...
| 后端隔离 | 无 flag 回归；native 调用图与运行命中诊断 | 防止默认路径回归或隐藏 VM |
...
本阶段不做通用多型号合成框架、多实例混音替代、音色库重制、并行 DSP 或全程序
