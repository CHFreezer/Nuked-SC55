...
| PCM 保留声音基础 | 从 metadata + 解码 PCM 建立等价输入；保持原厂数值与副作用，必要扩展仅限采样源接缝，见 16 |
| 原生运行零原 ROM 依赖 | 离线导入原素材生成自有 bank；原生启动、播放、复位与恢复只使用 bank，不读取原 ROM 文件或固件映像 |
| bank 是透明可编辑资产 | 公开 metadata + 无损解码 PCM 是优先模型；编辑后使用新素材，不预设需要原编码副本 |
...
mk2cpp/src/native/
  ...
  adapters/
    ...
    wav_sample_source.*        若取证表明需要，再增加采样源接缝；见 16 §7.4
...
bank 至少覆盖被恢复运行路径所需的音色/采样目录、参数/查找/初始化表及完整采样表示；
...
bank 以公开目录和版本化 JSON manifest 为资产形态，包含解码 PCM 波形、采样元数据及
具名数据表；先按 16 §7.4 验证表示完整性。记录素材身份、导入器版本、型号/格式版本、目录与
...
**M4.5 优先用公开 metadata + 无损解码 PCM 表示采样，提供 WAV 的试听、编辑和替换。**
运行中只从这两者建立等价的 PCM 输入，原编码留作取证参考，不规定为终态 bank 的
必需副本。若保留设备侧编码缓存，须能仅从当前 metadata/PCM 重新生成。
设备波形地址仍有实际用途，与 H8 程序地址无关；源资产中的引用由类型化 ID 表达。
...
| 对象 | 责任 |
|---|---|
| `WaveData` / bank 只读资产存储 | 精度明确的解码 PCM；同一份资产由所有 voice 共享，运行缓存是可重建产物 |
| `SampleDefinition` / 采样目录 | SampleId、波形路径/身份、采样率/基准音高、循环/初始化、经证明必要的恢复元数据 |
...
优先交付**无损解码 PCM + 公开元数据/数据表**的 bank；先验证元数据完整性及解码是否
存在不可逆舍入/截断，再确定存储精度。可逆性针对元数据与波形的组合验证，
WAV 无损存盘本身不足以证明原始解码可逆。具体证明方法见 16 §7.4。
WAV 不是装饰性预览，修改后重新加载必须生效；不能播放过期的编码缓存。

WAV 导出是正式功能，标明解码初值、方向/速率、数值精度与已验证范围。16-bit 预览
不能充当 20-bit 内部行为的无损依据，导出 WAV 本身也不保证原厂 bit 兼容。
优先证明从该资产模型恢复原 PCM 所需输入或等价采样计算；不预设双源为唯一架构。
确需扩展采样源时，按 16 §4 复用滤波/音量包络/混音/效果。源处理局部播放状态，
不直接访问 `pcm_t`，由 GT PCM 接入控制集成。
...
生效、派生缓存失效、音高/循环/控制与快照。修改后的素材不与原厂音频做 bit
