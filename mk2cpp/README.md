...
**原生运行零原 ROM 依赖**：离线导入原素材为自有 bank；启动、播放、复位和恢复只读取
bank。bank 必须提供标准 WAV 和公开音色/采样定义，支持独立试听、编辑、替换并重新
加载生效。未修改的原厂素材无损保留，stock 28 输出仍逐 bit 一致；编辑素材另做功能验收。
...
验收必须同时满足算法可读、行为等价，以及全生命周期脱离 MCU 指令执行和原 ROM 文件。
...
| **M4.5（待实施）** | **原生算法重建（stock 28）**：完整算法/状态/调度、透明 bank 与 WAV 替换 | 可读性 + 未修改原厂 bit 等价 + WAV 编辑生效 + 全生命周期零原 ROM/MCU 执行依赖 |
